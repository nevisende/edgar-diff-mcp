...
Generated 2026-09-03T01:10:39.222Z. **This is string-level grading only**: checks inspect the captured natural-language answer, not factual entailment or quote fidelity. Missing answer files and nonzero harness exits fail explicitly. The embedded prompt and command diagnostics are excluded from grading. Note that one Codex run (`xom-holding-company`) failed on a provider usage limit rather than on the task.

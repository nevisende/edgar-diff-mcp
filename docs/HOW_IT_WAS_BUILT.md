...
| Codex (GPT-5.6) | Five build lanes before the cold review plus six post-review lanes: eleven in total, all Codex. |
| Antigravity CLI (Claude Opus 4.6) | The live-corpus evaluation harness and the first failure-class investigation. Its quota ran out mid-night; Gemini took over the remaining Antigravity work. |
| Antigravity CLI (Claude Sonnet 4.6) | The CI workflow. A twenty-second job for a model that costs a fraction of the others. |
| Antigravity CLI (Gemini 3.8 Flash) | Third cold review; scenario harness and grader fixes; the navigation-furniture and 10-Q Item 2 parser fixes; scenario and supporting documentation updates. |
...
5. **A cold review.** When the work looked finished, two reviewers with no memory of the build (a fresh Claude session and a Codex session) were asked to judge the repository as a sceptical CTO would: the code, the tests, the docs, the claims. Their findings, and what changed because of them, are in the last section of this file.
6. **Run the scenario suite at two layers.** A deterministic MCP client first asserted the real tool pipelines, then the same seven analyst tasks ran through Claude, Codex, and Gemini harnesses and were checked by the grader. Reading the resulting diffs found two issues the corpus existence checks had missed: repeated navigation links in Exxon's risk factors and a Berkshire 10-Q Item 2 title variant that let repurchase tables leak into Item II.1A. Both fixes received fixture-backed coverage and the full 102-test suite stayed green.
7. **Nothing pushed by an agent.** `CLAUDE.md` forbids it, `.claude/settings.json` denies `git push`, and every commit is authored by me.
...
- **The headline number was recall dressed as precision.** "Found" meant "a key exists", so a 314-character Item 8 counted. The evaluation now reports two numbers, located and plausible-size, calls the metric recall, counts unexpected keys, and reports it as an issue (exit code stays 0) when an issuer yields fewer than two 10-Ks. The expected-Item lists were also incomplete (Item 9, 10-Q Items 3 to 5), which had produced 69 fake false positives.
...
A third pass, by Gemini 3.8 Flash through the Antigravity CLI, ran after those fixes and cost a few cents: nine findings, all small and all real, among them a `PART I ITEM 1` heading without punctuation being dropped, a lower-case form filter that would have paged through all of EDGAR before returning nothing, missing 10-Q titles in the fused-heading rule, and a quadratic page cutoff. All nine were fixed with tests; the evaluation numbers did not move.
